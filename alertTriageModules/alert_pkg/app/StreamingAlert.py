...
class StreamingAlert:
    ...
    def main(self):
        ...
        stream_data = alerts.withColumn(
            'date', col("window.start")).drop(*drop_col)
        pipelineModel = self.pipelineInit(staticDF=raw_data_init)
        full_stream_predictions = self.modelTraining(
        ...
        self.sendEmail(streamDF=stream_alerts_broadcast)

...
def main():
    ...
    raw_data = monit.consume(spark=spark,schema=schema,serverUrl="monit-kafka.cern.ch:9092")
    groupped_req_data = monit.windowGrouping(raw_data=raw_data,watermark="1 minute",timeInterval="1 minute",updatedTime="1 minute")
    monit.startMonit(streamingDF=groupped_req_data,hdfsPath="/cms/users/carizapo/ming/data_cmsweb_logs",checkpointLocation="/cms/users/carizapo/ming/req_checkpoint_cmsweb_logs")
